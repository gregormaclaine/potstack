...
import type { PlayerBreakdownRow, GroupBreakdownRow, BreakdownStatsItem } from '@/types';
import { isLinearAffectsError, type LinearAffectsResult } from '@/lib/computeLinearAffects';
...
interface BreakdownStatsViewProps {
  ...
  linearStrict: LinearAffectsResult;
  linearInclusive: LinearAffectsResult;
}
...
function ExplainerCard({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className='rounded-xl border border-zinc-800 bg-zinc-900/50'>
      ...
      >
        <span className='text-sm font-medium text-zinc-400'>How is this calculated?</span>
...
export default function BreakdownStatsView({
  ...
  const filteredGroupRows = useMemo(() => groupRows.filter(r => r.groupId !== -1), [groupRows]);
  ...
  const retryInMin =
    isRateLimited && currentLastRefreshedAt
      ? Math.ceil(
          (REFRESH_RATE_LIMIT_MS - (Date.now() - new Date(currentLastRefreshedAt).getTime())) / 60_000,
        )
      : 0;
  ...
  useEffect(() => {
    const isStale = lastRefreshedAt === null || Date.now() - new Date(lastRefreshedAt).getTime() > ONE_DAY_MS;
      ...
      <div>
        ...
        <div className='flex items-start justify-between'>
          ...
          <div className='flex flex-col items-end gap-2'>
            ...
            >
              <span className={clsx('text-sm leading-none', isPending && 'animate-spin inline-block')}>
                ↻
              </span>
              {isPending ? 'Refreshing…' : isRateLimited ? `Refresh (${retryInMin}m)` : 'Refresh stats'}
            </button>
          </div>
        </div>
      </div>
        ...
        <>
          ...
          <section className='space-y-4'>
            <div>
              ...
              <p className='mt-1 text-sm text-zinc-500'>
                How certain are we about each win rate? The bar shows the observed win rate; the whiskers show
                the 95% confidence interval from bootstrap resampling.
              </p>
            </div>
            ...
            <ExplainerCard title='Bootstrap Resampling'>
              <p>
                A <span className='text-zinc-200 font-medium'>win rate</span> is simply the fraction of
                sessions where you ended up with a profit. If you played 10 sessions and won 6, your win rate
                is 60%. But with only 10 sessions, that 60% could easily be 45% or 75% if you ran those
                sessions again — the sample is too small to be certain.
              </p>
              <p>
                <span className='text-zinc-200 font-medium'>Bootstrap resampling</span> quantifies that
                uncertainty. The method works by repeatedly drawing new fake samples of the same size from
                your actual sessions (sampling with replacement — the same session can be picked more than
                once), and computing the win rate for each fake sample. After 5,000 such samples, we take the
                2.5th and 97.5th percentiles of those win rates as the lower and upper bound of the{' '}
                <span className='text-zinc-200 font-medium'>95% confidence interval</span>.
              </p>
              <p>
                A wide interval means your estimate is uncertain — usually because you have few sessions with
                that player. A narrow interval means the data is telling a more consistent story. The true win
                rate has a 95% chance of falling within the shown range, assuming your future sessions
                resemble your past ones.
              </p>
            </ExplainerCard>
          </section>
          ...
          <section className='space-y-4'>
            <div>
              ...
              <p className='mt-1 text-sm text-zinc-500'>
                The estimated probability that a randomly chosen future session with each player/group will
                result in a profit. Green bars are above 50%; red bars below.
              </p>
            </div>
            ...
            <ExplainerCard title='Normal Distribution Modelling'>
              <p>
                ...
                <span className='text-zinc-200 font-medium'>probability of profit</span> uses the actual
                profit amounts to make a more nuanced estimate. Rather than just counting wins, we model the
                distribution of session profits using a{' '}
                <span className='text-zinc-200 font-medium'>normal distribution</span> — a bell-curve fitted
                to your past session results.
              </p>
              <p>
                From your sessions we compute two things: the{' '}
                <span className='text-zinc-200 font-medium'>mean profit</span> (average result per session)
                and the <span className='text-zinc-200 font-medium'>standard deviation</span> (how spread out
                the results are). We then ask: given a bell-curve with that mean and spread, what fraction of
                it sits above zero? That fraction is P(Profit).
              </p>
              <p>
                Formally, if X ~ N(μ, σ²), then P(X &gt; 0) = Φ(μ / σ), where Φ is the standard normal
                cumulative distribution function. In plain terms: the higher and tighter your profit
                distribution, the closer this value is to 100%. If your mean profit is negative or your
                variance is very large relative to your mean, it will be much lower.
              </p>
              <p>
                Note that this is a <span className='text-zinc-200 font-medium'>parametric</span> estimate (it
                assumes a normal distribution) whereas the win-rate CI above is{' '}
                <span className='text-zinc-200 font-medium'>non-parametric</span> (it makes no assumption
                about the shape of your profit distribution).
              </p>
            </ExplainerCard>
          </section>
          ...
          {(() => {
            ...
            return (
              <section className='space-y-4'>
                ...
                {isLinearAffectsError(currentLinear) ? (
                  <div className='rounded-xl border border-dashed border-zinc-700 py-12 text-center'>
                    <p className='font-medium text-zinc-400'>Model could not be fit</p>
                    <p className='mt-2 text-sm text-zinc-500'>{currentLinear.error}</p>
                  </div>
                ) : (
                  <>
                    <div className='rounded-xl border border-zinc-800 bg-zinc-900 p-5'>
                    ...
                    {currentLinear.excludedPlayers.length > 0 && (
                      <p className='text-xs italic text-zinc-500'>
                        {currentLinear.excludedPlayers.map(p => p.playerName).join(', ')}{' '}
                        {currentLinear.excludedPlayers.length === 1 ? 'was' : 'were'} excluded —{' '}
                        {currentLinear.excludedPlayers.length === 1 ? 'they appear' : 'all appear'} in every
                        included session, making them indistinguishable from the model baseline.
                      </p>
                    )}
                    ...
                    <ExplainerCard title='OLS Linear Main-Effects Model'>
                      <p>
                        ...
                        <span className='text-zinc-200 font-medium'>marginal effect</span> of each player on
                        your session profit using{' '}
                      ...
                      <p>
                        The model is <span className='text-zinc-200 font-mono'>Y = β₀ + β₁x₁ + … + βₙxₙ</span>
                        , where Y is your profit and each xᵢ is 1 if player i was present, 0 if not. Only
                        players with at least 3 sessions appear as features.
                      </p>
                      <p>
                        Each bar shows the predicted profit when{' '}
                        <span className='text-zinc-200 font-medium'>only that one player is present</span> —
                        formally, the model evaluated at the unit vector eᵢ. This is not the average profit
                        when that player appears; it isolates their contribution while holding all others at
                        zero.
                      </p>
                      <p>
                        The whiskers are a{' '}
                        ...
                        (not a prediction interval for a single future session). A wide interval means few
                        sessions with that player, or that they rarely appear independently of others.
                      </p>
                      <p>
                        The{' '}
                        <span className='text-zinc-200 font-medium'>
                        ...
                        controls whether sessions containing non-qualifying players (fewer than 3 sessions)
                        are included in the fit. Excluding them keeps the model cleaner but reduces the data
                        available.
                      </p>
                      <p>
                        The model assumes <span className='text-zinc-200 font-medium'>additive effects</span>{' '}
                        with no interaction terms. Treat estimates as indicative, not causal — a negative
                        ...
                        <table className='w-full min-w-max text-sm'>
                          ...
                          <tbody className='divide-y divide-zinc-800'>
                            {[...currentLinear.players]
                              ...
                              .map(player => (
                                <tr
                                  key={player.playerIds.join('-')}
                                  className='bg-zinc-950 hover:bg-zinc-900/40'
                                >
                                  <td className='px-4 py-3 font-medium text-zinc-200'>{player.playerName}</td>
