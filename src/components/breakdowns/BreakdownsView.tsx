...
import type { UnifiedSession, PlayerGroup, PlayerBreakdownRow, GroupBreakdownRow } from '@/types';
...
interface BreakdownsViewProps {
  ...
  playerMetas: [number, { name: string; group: PlayerGroup | null }][];
...
export default function BreakdownsView({
  ...
  const playerRows = useMemo(() => buildPlayerBreakdowns(sessions), [sessions]);
  ...
  const isEmpty =
    mode === 'player' ? playerRows.length === 0 : groupRows.filter(r => r.groupId !== -1).length === 0;
      ...
      <div className='flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between'>
        <div className='flex items-start justify-between sm:justify-start gap-3'>
          <div>
            ...
            <p className='mt-1 text-sm text-zinc-500'>Your session stats grouped by player or group</p>
          </div>
          <div className='group relative mt-1.5'>
            ...
            <div className='pointer-events-none absolute left-1/2 top-7 z-10 w-64 -translate-x-1/2 rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-xs text-zinc-400 opacity-0 shadow-lg transition-opacity group-hover:opacity-100'>
              <p>
                <span className='font-semibold text-zinc-200'>By Player</span> only shows players who have
                appeared in at least 3 of your sessions.
              </p>
              <p className='mt-1'>
                <span className='font-semibold text-zinc-200'>By Group</span> includes a session under a group
                when at least half of the grouped players in that session belong to it.
              </p>
            </div>
          </div>
        ...
        <>
                  ...
                  ? sortedPlayerRows.map(row => (
                      <tr key={row.playerId} className='bg-zinc-950 hover:bg-zinc-900/50'>
                        ...
                        <td className='px-4 py-3 text-right text-sm text-zinc-400'>
                          {formatCurrency(row.totalBuyIn)}
                        </td>
                        <td className='px-4 py-3 text-right text-sm text-zinc-400'>
                          {formatCurrency(row.totalCashOut)}
                        </td>
                        <td className='px-4 py-3 text-right'>
                          <Badge value={row.profit} />
                        </td>
                        <td className='px-4 py-3 text-right'>
                          <Badge value={row.avgProfit} />
                        </td>
                        <td className='px-4 py-3 text-right'>
                          <Badge value={row.maxProfit} />
                        </td>
                        <td className='px-4 py-3 text-right'>
                          <Badge value={row.maxLoss} />
                        </td>
                        <td className='px-4 py-3 text-right text-sm text-zinc-400'>
          ...
          <div className='grid grid-cols-1 gap-4 lg:grid-cols-2'>
            <div className='rounded-xl border border-zinc-800 bg-zinc-900 p-4'>
              <h2 className='mb-3 text-sm font-semibold text-zinc-300'>Avg Profit per Session</h2>
              <AvgProfitChart data={chartData} />
            </div>
            <div className='rounded-xl border border-zinc-800 bg-zinc-900 p-4'>
              <h2 className='mb-3 text-sm font-semibold text-zinc-300'>Win Rate</h2>
